...
$ (function () {
  ...
  $ ('#startGame').click (function (e) {
    ...
    // hide buttons
    $ ('#endGameButton').addClass ("hidden").on('click', function () {
      endGame();
    });
    ...
    function createNewGameItem (randomItem) {
      ...
      return $ ('<div></div>').addClass ('gameItem').css ({
    ...
    function showAndHideElement (newItem) {
      newItem.appendTo ('body').delay (12000).queue (function () {
        $ (this).remove ();
      })
    };
    ...
    function initGame () {
      let intervals = [];
      intervals.push(initLevel (1.5, 0));

      const timeout1 = setTimeout (() => {
        intervals.push(nextLevel (2, 0));
      }, 20000);
      const timeout2 = setTimeout (() => {
        intervals.push(nextLevel (4, 0));
      }, 40000);
